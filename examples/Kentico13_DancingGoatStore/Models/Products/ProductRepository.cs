...
namespace DancingGoat.Models
{
    ...
    public class ProductRepository
    {
        ...
        /// </summary>        
        /// <param name="nodeGUIDs">Collection of the product nodes identifiers.</param>
        public IEnumerable<SKUTreeNode> GetProducts(ICollection<Guid> nodeGUIDs)
        {
            var GUIDList = nodeGUIDs.ToList();
            return pageRetriever.Retrieve<TreeNode>(
                query => query
                    .WhereIn("NodeGUID", nodeGUIDs)
                    .OrderBy(product => GUIDList.IndexOf(product.NodeGUID)),
                cache => cache
                    .Key($"{nameof(ProductRepository)}|{nameof(GetProducts)}|{String.Join(";", nodeGUIDs)}")
                    .Dependencies((_, builder) => builder.ObjectType("ecommerce.sku")))
                .Select(page => page as SKUTreeNode)
                .OrderBy(product => GUIDList.IndexOf(product.NodeGUID));
        }
